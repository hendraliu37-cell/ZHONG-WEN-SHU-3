// stt — speech-to-text for the translator, via Azure Speech (REST, short audio).
// Accepts a base64 WAV (PCM mono — what the client records), reads the sample
// rate from the RIFF header so any rate works, and returns the transcript.
// Key stays server-side (AZURE_SPEECH_KEY/REGION); requires a Supabase user JWT.
...
// Little-endian uint from a RIFF/WAV header. Returns 16000 if it can't be read.
...
Deno.serve(async (req: Request) => {
  ...
  let payload: { audio?: string; lang?: string };
  try {
