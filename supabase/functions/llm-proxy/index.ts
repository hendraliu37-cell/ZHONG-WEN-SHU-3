...
function buildSystemPrompt(track: string, level: string): string {
  const variant = track === "traditional" ? "Tradisional (hanzi tradisional + 注音)" : "Sederhana (hanzi sederhana + pinyin bertanda nada)";
  return `Kamu adalah Guru (老师), tutor bahasa Mandarin yang sabar, cerdas, dan humoris untuk penutup Bahasa Indonesia.
...
2. Struktur jawaban: respons natural → hanzi + pinyin + arti → follow-up singkat
3. Format kata baru: 汉字 (pīnyīn) = "arti"
4. Beri 1-2 contoh kalimat natural
5. JANGAN: daftar panjang, placeholder, "PR-mu kucatat ke rapor", EMOJI
...
Guru: "Nah pertanyaan bagus! 想 (xiǎng) = ingin/mau, 觉得 (juéde) = merasa/berpendapat. 我想吃饭 = aku mau makan. 我觉得很好吃 = menurutku enak. Gampangnya: 想 = want, 觉得 = think."
...
Deno.serve(async (req: Request) => {
  ...
  let payload: { messages?: unknown; track?: string; level?: string; model?: string };
  try {
  ...
  const level = typeof payload.level === "string" ? payload.level : "HSK 1-2 / TOCFL Band A";
  ...
  const modelList: string[] = ["deepseek-v4-flash", "deepseek-v4-flash-free", "deepseek-chat", "deepseek-v3"];
  ...
  for (const model of models) {
    try {
      const isMessages = usingOpenModel || baseRaw.includes("openmodel.ai") || messagesEndpoint === chatEndpoint;
      ...
      if (resp.status === 401 || resp.status === 403) {
        ...
        return json({ error: "auth_failed", status: resp.status, detail: text.slice(0, 300) }, 502);
      }
  ...
  return json({ error: "all_models_failed", detail: lastError.slice(0, 500), tried: models }, 502);
});
